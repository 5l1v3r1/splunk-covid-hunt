...
def main():
    ...
    if confirm == 'y':
        ...
        if args.noapi:
            ...
            baseurl = baseurl.replace('https', 'http', 1) #REMOVE
            ...
            if login == 0:
                ...
                if not sids:
                    ...
                    for sid in sids:
                        print("Search completed! Your results can be found at " + str(splunk_addr).replace("https","http") + "/app/search/search?sid=" + str(sid)) #REMOVE .replace()     
            else:
...
def search(username, password, baseurl, outfile, ioc_array, num_terms, earliest, latest):
    ...
    requests.packages.urllib3.disable_warnings() #REMOVE
    results = []
    i = 0
...
def slack_bot_webhook(webhook, baseurl, num_results=None, job_sid=''):
    ...
    if not job_sid:
        ...
        job_sid = "Your results can be found at " + str(baseurl).replace("https","http") + "/app/search/search?sid=" + str(job_sid) #REMOVE .replace()
    if num_results:
